							...
							LogicalRewriteMappingData *map)
{
	...
	if (!found)
	{
		...
		snprintf(path, MAXPGPATH,
				 "mdb_logical/mappings/" LOGICAL_REWRITE_FORMAT,
				 dboid, relid,
...
CheckPointLogicalRewriteHeap(void)
{
	...
	mappings_dir = AllocateDir("mdb_logical/mappings");
	while ((mapping_de = ReadDir(mappings_dir, "mdb_logical/mappings")) != NULL)
	{
		struct stat statbuf;
		Oid			dboid;
		Oid			relid;
		...
		snprintf(path, MAXPGPATH, "mdb_logical/mappings/%s", mapping_de->d_name);
		...
		{
			int			fd = OpenTransientFile(path, O_RDONLY | MDB_BINARY, 0);
