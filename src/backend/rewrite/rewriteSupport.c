 ...
 *		Set the value of the relation's relhasrules field in mdb_class.
 *
...
SetRelationRuleStatus(Oid relationId, bool relHasRules)
{
	...
	/*
	 * Find the tuple to update in mdb_class, using syscache for the lookup.
	 */
...
get_rewrite_oid(Oid relid, const char *rulename, bool missing_ok)
{
	...
	/* Find the rule's mdb_rewrite tuple, get its OID */
	...
	Assert(relid == ((Form_mdb_rewrite) GETSTRUCT(tuple))->ev_class);
							  ...
							  Oid *reloid, bool missing_ok)
{
	...
	/* Search mdb_rewrite for such a rule */
	...
	else
	{
		...
		if (reloid != NULL)
			*reloid = ((Form_mdb_rewrite) GETSTRUCT(htup))->ev_class;
