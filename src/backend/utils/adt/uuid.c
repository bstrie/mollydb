...
Datum
uuid_in(MDB_FUNCTION_ARGS)
{
	char	   *uuid_str = MDB_GETARG_CSTRING(0);
	mdb_uuid_t  *uuid;

	uuid = (mdb_uuid_t *) palloc(sizeof(*uuid));
	string_to_uuid(uuid_str, uuid);
	MDB_RETURN_UUID_P(uuid);
}

Datum
uuid_out(MDB_FUNCTION_ARGS)
{
	mdb_uuid_t  *uuid = MDB_GETARG_UUID_P(0);
...
Datum
uuid_recv(MDB_FUNCTION_ARGS)
{
	StringInfo	buffer = (StringInfo) MDB_GETARG_POINTER(0);
	mdb_uuid_t  *uuid;

	uuid = (mdb_uuid_t *) palloc(UUID_LEN);
	memcpy(uuid->data, pq_getmsgbytes(buffer, UUID_LEN), UUID_LEN);
	MDB_RETURN_POINTER(uuid);
}

Datum
uuid_send(MDB_FUNCTION_ARGS)
{
	mdb_uuid_t  *uuid = MDB_GETARG_UUID_P(0);
	...
	MDB_RETURN_BYTEA_P(pq_endtypsend(&buffer));
}
...
Datum
uuid_lt(MDB_FUNCTION_ARGS)
{
	mdb_uuid_t  *arg1 = MDB_GETARG_UUID_P(0);
	mdb_uuid_t  *arg2 = MDB_GETARG_UUID_P(1);

	MDB_RETURN_BOOL(uuid_internal_cmp(arg1, arg2) < 0);
}

Datum
uuid_le(MDB_FUNCTION_ARGS)
{
	mdb_uuid_t  *arg1 = MDB_GETARG_UUID_P(0);
	mdb_uuid_t  *arg2 = MDB_GETARG_UUID_P(1);
...
Datum
uuid_cmp(MDB_FUNCTION_ARGS)
{
	mdb_uuid_t  *arg1 = MDB_GETARG_UUID_P(0);
	mdb_uuid_t  *arg2 = MDB_GETARG_UUID_P(1);

	MDB_RETURN_INT32(uuid_internal_cmp(arg1, arg2));
}
 ...
 */
Datum
uuid_sortsupport(MDB_FUNCTION_ARGS)
{
...
/* hash index support */
Datum
uuid_hash(MDB_FUNCTION_ARGS)
{
	mdb_uuid_t  *key = MDB_GETARG_UUID_P(0);
