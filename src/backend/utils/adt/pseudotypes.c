...
/*
 * mdb_node_tree_in		- input routine for type PG_NODE_TREE.
 *
 ...
 */
Datum
mdb_node_tree_in(PG_FUNCTION_ARGS)
{
	/*
...
/*
 * mdb_node_tree_out		- output routine for type PG_NODE_TREE.
 *
 * The internal representation is the same as TEXT, so just pass it off.
 */
Datum
mdb_node_tree_out(PG_FUNCTION_ARGS)
{
	return textout(fcinfo);
}

/*
 * mdb_node_tree_recv		- binary input routine for type PG_NODE_TREE.
 */
Datum
mdb_node_tree_recv(PG_FUNCTION_ARGS)
{
	ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			 errmsg("cannot accept a value of type %s", "mdb_node_tree")));
...
/*
 * mdb_node_tree_send		- binary output routine for type PG_NODE_TREE.
 */
Datum
mdb_node_tree_send(PG_FUNCTION_ARGS)
{
	return textsend(fcinfo);
}

/*
 * mdb_ddl_command_in	- input routine for type PG_DDL_COMMAND.
 *
 * Like mdb_node_tree, mdb_ddl_command isn't really a pseudotype; it's here for
 * the same reasons as that one.
 */
Datum
mdb_ddl_command_in(PG_FUNCTION_ARGS)
{
	/*
...
/*
 * mdb_ddl_command_out		- output routine for type PG_DDL_COMMAND.
 *
 * We don't have any good way to output this type directly, so punt.
 */
Datum
mdb_ddl_command_out(PG_FUNCTION_ARGS)
{
	ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
...
/*
 * mdb_ddl_command_recv		- binary input routine for type PG_DDL_COMMAND.
 */
Datum
mdb_ddl_command_recv(PG_FUNCTION_ARGS)
{
	ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			 errmsg("cannot accept a value of type %s", "mdb_ddl_command")));
...
/*
 * mdb_ddl_command_send		- binary output routine for type PG_DDL_COMMAND.
 */
Datum
mdb_ddl_command_send(PG_FUNCTION_ARGS)
{
	ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			 errmsg("cannot output a value of type %s", "mdb_ddl_command")));
