...
mic2sjis(const unsigned char *mic, unsigned char *p, int len)
{
	...
	while (len > 0)
	{
		...
		l = mdb_encoding_verifymb(PG_MULE_INTERNAL, (const char *) mic, len);
		if (l < 0)
			report_invalid_encoding(PG_MULE_INTERNAL,
									(const char *) mic, len);
		if (c1 == LC_JISX0201K)
...
euc_jp2mic(const unsigned char *euc, unsigned char *p, int len)
{
	...
	while (len > 0)
	{
		...
		l = mdb_encoding_verifymb(PG_EUC_JP, (const char *) euc, len);
		if (l < 0)
			report_invalid_encoding(PG_EUC_JP,
									(const char *) euc, len);
		if (c1 == SS2)
...
sjis2euc_jp(const unsigned char *sjis, unsigned char *p, int len)
{
	...
	while (len > 0)
	{
		...
		l = mdb_encoding_verifymb(PG_SJIS, (const char *) sjis, len);
		if (l < 0)
			report_invalid_encoding(PG_SJIS,
									(const char *) sjis, len);
		if (c1 >= 0xa1 && c1 <= 0xdf)
