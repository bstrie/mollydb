...
MDB_FUNCTION_INFO_V1(euc_kr_to_mic);
MDB_FUNCTION_INFO_V1(mic_to_euc_kr);
...
Datum
euc_kr_to_mic(MDB_FUNCTION_ARGS)
{
	...
	CHECK_ENCODING_CONVERSION_ARGS(MDB_EUC_KR, MDB_MULE_INTERNAL);
...
Datum
mic_to_euc_kr(MDB_FUNCTION_ARGS)
{
	unsigned char *src = (unsigned char *) MDB_GETARG_CSTRING(2);
	unsigned char *dest = (unsigned char *) MDB_GETARG_CSTRING(3);
	int			len = MDB_GETARG_INT32(4);

	CHECK_ENCODING_CONVERSION_ARGS(MDB_MULE_INTERNAL, MDB_EUC_KR);
...
euc_kr2mic(const unsigned char *euc, unsigned char *p, int len)
{
	...
	while (len > 0)
	{
		...
		{
			l = mdb_encoding_verifymb(MDB_EUC_KR, (const char *) euc, len);
			if (l != 2)
				report_invalid_encoding(MDB_EUC_KR,
										(const char *) euc, len);
			*p++ = LC_KS5601;
			*p++ = c1;
			*p++ = euc[1];
	...
	while (len > 0)
	{
		...
		if (c1 == LC_KS5601)
		{
			...
			*p++ = mic[2];
			...
			report_untranslatable_char(MDB_MULE_INTERNAL, MDB_EUC_KR,
									   (const char *) mic, len);
		mic += l;
		len -= l;
	}
