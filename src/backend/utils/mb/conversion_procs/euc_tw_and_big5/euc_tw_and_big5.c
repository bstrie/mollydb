...
MDB_FUNCTION_INFO_V1(euc_tw_to_big5);
MDB_FUNCTION_INFO_V1(big5_to_euc_tw);
MDB_FUNCTION_INFO_V1(euc_tw_to_mic);
MDB_FUNCTION_INFO_V1(mic_to_euc_tw);
MDB_FUNCTION_INFO_V1(big5_to_mic);
MDB_FUNCTION_INFO_V1(mic_to_big5);
...
Datum
euc_tw_to_big5(MDB_FUNCTION_ARGS)
{
	...
	CHECK_ENCODING_CONVERSION_ARGS(MDB_EUC_TW, MDB_BIG5);
...
Datum
big5_to_euc_tw(MDB_FUNCTION_ARGS)
{
	unsigned char *src = (unsigned char *) MDB_GETARG_CSTRING(2);
	unsigned char *dest = (unsigned char *) MDB_GETARG_CSTRING(3);
	int			len = MDB_GETARG_INT32(4);
	...
	CHECK_ENCODING_CONVERSION_ARGS(MDB_BIG5, MDB_EUC_TW);
...
Datum
euc_tw_to_mic(MDB_FUNCTION_ARGS)
{
	unsigned char *src = (unsigned char *) MDB_GETARG_CSTRING(2);
	unsigned char *dest = (unsigned char *) MDB_GETARG_CSTRING(3);
	int			len = MDB_GETARG_INT32(4);

	CHECK_ENCODING_CONVERSION_ARGS(MDB_EUC_TW, MDB_MULE_INTERNAL);
...
Datum
mic_to_euc_tw(MDB_FUNCTION_ARGS)
{
	unsigned char *src = (unsigned char *) MDB_GETARG_CSTRING(2);
	unsigned char *dest = (unsigned char *) MDB_GETARG_CSTRING(3);
	int			len = MDB_GETARG_INT32(4);

	CHECK_ENCODING_CONVERSION_ARGS(MDB_MULE_INTERNAL, MDB_EUC_TW);
...
Datum
big5_to_mic(MDB_FUNCTION_ARGS)
{
	unsigned char *src = (unsigned char *) MDB_GETARG_CSTRING(2);
	unsigned char *dest = (unsigned char *) MDB_GETARG_CSTRING(3);
	int			len = MDB_GETARG_INT32(4);

	CHECK_ENCODING_CONVERSION_ARGS(MDB_BIG5, MDB_MULE_INTERNAL);
...
Datum
mic_to_big5(MDB_FUNCTION_ARGS)
{
	unsigned char *src = (unsigned char *) MDB_GETARG_CSTRING(2);
	unsigned char *dest = (unsigned char *) MDB_GETARG_CSTRING(3);
	int			len = MDB_GETARG_INT32(4);

	CHECK_ENCODING_CONVERSION_ARGS(MDB_MULE_INTERNAL, MDB_BIG5);
...
euc_tw2mic(const unsigned char *euc, unsigned char *p, int len)
{
	...
	while (len > 0)
	{
		...
		{
			l = mdb_encoding_verifymb(MDB_EUC_TW, (const char *) euc, len);
			if (l < 0)
				report_invalid_encoding(MDB_EUC_TW,
										(const char *) euc, len);
			if (c1 == SS2)
			{
				c1 = euc[1];	/* plane No. */
...
mic2euc_tw(const unsigned char *mic, unsigned char *p, int len)
{
	...
	while (len > 0)
	{
		...
		l = mdb_encoding_verifymb(MDB_MULE_INTERNAL, (const char *) mic, len);
		if (l < 0)
			report_invalid_encoding(MDB_MULE_INTERNAL,
									(const char *) mic, len);
		if (c1 == LC_CNS11643_1)
				 ...
				 mic[1] >= LC_CNS11643_3 && mic[1] <= LC_CNS11643_7)
		{
			...
			*p++ = mic[3];
			...
			report_untranslatable_char(MDB_MULE_INTERNAL, MDB_EUC_TW,
									   (const char *) mic, len);
		mic += l;
		len -= l;
	}
...
big52mic(const unsigned char *big5, unsigned char *p, int len)
{
	...
	while (len > 0)
	{
		...
		if (!IS_HIGHBIT_SET(c1))
		{
			/* ASCII */
			if (c1 == 0)
				report_invalid_encoding(MDB_BIG5,
										(const char *) big5, len);
			*p++ = c1;
			big5++;
			len--;
		...
		l = mdb_encoding_verifymb(MDB_BIG5, (const char *) big5, len);
		if (l < 0)
			report_invalid_encoding(MDB_BIG5,
									(const char *) big5, len);
		big5buf = (c1 << 8) | big5[1];
		...
		if (lc != 0)
		{
			...
			*p++ = cnsBuf & 0x00ff;
			...
			report_untranslatable_char(MDB_BIG5, MDB_MULE_INTERNAL,
									   (const char *) big5, len);
		big5 += l;
		len -= l;
	}
...
mic2big5(const unsigned char *mic, unsigned char *p, int len)
{
	...
	while (len > 0)
	{
		...
		if (c1 == LC_CNS11643_1 || c1 == LC_CNS11643_2 || c1 == LCPRV2_B)
		{
			...
			if (big5buf == 0)
				report_untranslatable_char(MDB_MULE_INTERNAL, MDB_BIG5,
										   (const char *) mic, len);
			*p++ = (big5buf >> 8) & 0x00ff;
			*p++ = big5buf & 0x00ff;
		}
