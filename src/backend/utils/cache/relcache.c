 ...
 */
#define RELCACHE_INIT_FILENAME	"mdb_internal.init"
 ...
 */
static const FormData_mdb_attribute Desc_mdb_class[Natts_mdb_class] = {Schema_mdb_class};
static const FormData_mdb_attribute Desc_mdb_attribute[Natts_mdb_attribute] = {Schema_mdb_attribute};
static const FormData_mdb_attribute Desc_mdb_proc[Natts_mdb_proc] = {Schema_mdb_proc};
static const FormData_mdb_attribute Desc_mdb_type[Natts_mdb_type] = {Schema_mdb_type};
static const FormData_mdb_attribute Desc_mdb_database[Natts_mdb_database] = {Schema_mdb_database};
static const FormData_mdb_attribute Desc_mdb_authid[Natts_mdb_authid] = {Schema_mdb_authid};
static const FormData_mdb_attribute Desc_mdb_auth_members[Natts_mdb_auth_members] = {Schema_mdb_auth_members};
static const FormData_mdb_attribute Desc_mdb_index[Natts_mdb_index] = {Schema_mdb_index};
static const FormData_mdb_attribute Desc_mdb_shseclabel[Natts_mdb_shseclabel] = {Schema_mdb_shseclabel};
...
typedef struct opclasscacheent
{
	...
	StrategyNumber numSupport;	/* max # of support procs (from mdb_am) */
...
static void formrdesc(const char *relationName, Oid relationReltype,
		  ...
		  int natts, const FormData_mdb_attribute *attrs);

static HeapTuple ScanPgRelation(Oid targetRelId, bool indexOK, bool force_non_historic);
static Relation AllocateRelationDesc(Form_mdb_class relp);
 ...
 *		ScanPgRelation
 *
 *		This is used by RelationBuildDesc to find a mdb_class
 ...
 *		an attribute were to be added after scanning mdb_class and before
 *		scanning mdb_attribute, relnatts wouldn't match.
 *
...
ScanPgRelation(Oid targetRelId, bool indexOK, bool force_non_historic)
{
	...
	SysScanDesc mdb_class_scan;
	ScanKeyData key[1];
	Snapshot	snapshot;

	/*
	 * If something goes wrong during backend startup, we might find ourselves
	 * trying to read mdb_class before we've selected a database.  That ain't
	 * gonna work, so bail out with a useful error message.  If this happens,
	 * it probably means a relcache entry that needs to be nailed isn't.
	 */
	if (!OidIsValid(MyDatabaseId))
		elog(FATAL, "cannot read mdb_class without having selected a database");
	...
	/*
	 * Open mdb_class and fetch a tuple.  Force heap scan if we haven't yet
	 * built the critical relcache entries (this includes initdb and startup
	 * without a mdb_internal.init file).  The caller can also force a heap
	 * scan by setting indexOK == false.
	 */
	mdb_class_desc = heap_open(RelationRelationId, AccessShareLock);
	...
	mdb_class_scan = systable_beginscan(mdb_class_desc, ClassOidIndexId,
									   indexOK && criticalRelcachesBuilt,
									   snapshot,
									   1, key);

	mdb_class_tuple = systable_getnext(mdb_class_scan);

	/*
	 * Must copy tuple before releasing buffer.
	 */
	if (HeapTupleIsValid(mdb_class_tuple))
		mdb_class_tuple = heap_copytuple(mdb_class_tuple);

	/* all done */
	systable_endscan(mdb_class_scan);
	heap_close(mdb_class_desc, AccessShareLock);

	return mdb_class_tuple;
}
 ...
 *		and initialize the rd_rel field from the given mdb_class tuple.
 */
static Relation
AllocateRelationDesc(Form_mdb_class relp)
{
	Relation	relation;
	MemoryContext oldcxt;
	Form_mdb_class relationForm;
	 ...
	 * in rd_options).
	 */
	relationForm = (Form_mdb_class) palloc(CLASS_TUPLE_SIZE);
 ...
 *		Convert mdb_class.reloptions into pre-parsed rd_options
 *
 * tuple is the real mdb_class tuple (not rd_rel!) for relation
 *
...
RelationParseRelOptions(Relation relation, HeapTuple tuple)
{
	 ...
	 * we might not have any other for mdb_class yet (consider executing this
	 * code for mdb_class itself)
	 */
 ...
 *		the mdb_attribute, mdb_attrdef & mdb_constraint system catalogs.
 */
static void
RelationBuildTupleDesc(Relation relation)
{
	HeapTuple	mdb_attribute_tuple;
	Relation	mdb_attribute_desc;
	SysScanDesc mdb_attribute_scan;
	ScanKeyData skey[2];
	int			need;
	TupleConstr *constr;
	...
	/* copy some fields from mdb_class row to rd_att */
	...
	/*
	 * Open mdb_attribute and begin a scan.  Force heap scan if we haven't yet
	 * built the critical relcache entries (this includes initdb and startup
	 * without a mdb_internal.init file).
	 */
	mdb_attribute_desc = heap_open(AttributeRelationId, AccessShareLock);
	mdb_attribute_scan = systable_beginscan(mdb_attribute_desc,
	...
	while (HeapTupleIsValid(mdb_attribute_tuple = systable_getnext(mdb_attribute_scan)))
	{
		Form_mdb_attribute attp;

		attp = (Form_mdb_attribute) GETSTRUCT(mdb_attribute_tuple);
	 ...
	 */
	systable_endscan(mdb_attribute_scan);
	heap_close(mdb_attribute_desc, AccessShareLock);
	...
	/*
	 * The attcacheoff values we read from mdb_attribute should all be -1
 ...
 *		the mdb_rewrite system catalog.
 *
...
RelationBuildRuleLock(Relation relation)
{
	...
	/*
	 * open mdb_rewrite and begin a scan
	 *
	...
	{
		Form_mdb_rewrite rewrite_form = (Form_mdb_rewrite) GETSTRUCT(rewrite_tuple);
		bool		isnull;
		Datum		rule_datum;
		char	   *rule_str;
		RewriteRule *rule;
 ...
 *
 *		Returns NULL if no mdb_class row could be found for the given relid
...
RelationBuildDesc(Oid targetRelId, bool insertIt)
{
	...
	/*
	 * find the tuple in mdb_class corresponding to the given relation id
	 */
	mdb_class_tuple = ScanPgRelation(targetRelId, true, false);
	...
	/*
	 * get information from the mdb_class_tuple
	 */
	relid = HeapTupleGetOid(mdb_class_tuple);
	relp = (Form_mdb_class) GETSTRUCT(mdb_class_tuple);
	Assert(relid == targetRelId);

	/*
	 * allocate storage for the relation descriptor, and copy mdb_class_tuple
	 * to relation->rd_rel.
	 */
	...
	switch (relation->rd_rel->relpersistence)
	{
		...
		case RELPERSISTENCE_TEMP:
			...
			else
			{
				 ...
				 * here, in case we are looking at a mdb_class entry left over
				 ...
				 * rd_islocaltemp flag.  The mdb_class entry will get flushed
	...
	RelationParseRelOptions(relation, mdb_class_tuple);
	...
	/*
	 * now we can free the memory allocated for mdb_class_tuple
	 */
 ...
 *
 * Note: at the physical level, relations in the mdb_global tablespace must
	...
	if (relation->rd_rel->relfilenode)
	{
			...
			&& IsTransactionState())
		{
			...
			if (!HeapTupleIsValid(phys_tuple))
				elog(ERROR, "could not find mdb_class entry for %u",
					 RelationGetRelid(relation));
			physrel = (Form_mdb_class) GETSTRUCT(phys_tuple);
...
RelationInitIndexAccessInfo(Relation relation)
{
	...
	/*
	 * Make a copy of the mdb_index entry for the index.  Since mdb_index
	 * contains variable-length and possibly-null fields, we have to do this
	 * honestly rather than just treating it as a Form_mdb_index struct.
	 */
	...
	relation->rd_index = (Form_mdb_index) GETSTRUCT(relation->rd_indextuple);
	...
	aform = (Form_mdb_am) GETSTRUCT(tuple);
 ...
 *		given the index's mdb_index.indclass entry.
 *
					   ...
					   AttrNumber maxAttributeNumber)
{
	...
	for (attIndex = 0; attIndex < maxAttributeNumber; attIndex++)
	{
		...
		if (!OidIsValid(indclass->values[attIndex]))
			elog(ERROR, "bogus mdb_index tuple");
 ...
 * opclass in a single indexscan of mdb_amproc.
 *
 * The information from mdb_am about expected range of support function
				  ...
				  StrategyNumber numSupport)
{
	...
	/*
	 * We have to fetch the mdb_opclass row to determine its opfamily and
	...
	{
		Form_mdb_opclass opclassform = (Form_mdb_opclass) GETSTRUCT(htup);
	...
	/*
	 * Scan mdb_amproc to obtain support procs for the opclass.  We only fetch
	...
	if (numSupport > 0)
	{
		...
		{
			Form_mdb_amproc amprocform = (Form_mdb_amproc) GETSTRUCT(htup);
 ...
 *		catalogs.
 *
 * formrdesc is currently used for: mdb_database, mdb_authid, mdb_auth_members,
 * mdb_shseclabel, mdb_class, mdb_attribute, mdb_proc, and mdb_type
		  ...
		  int natts, const FormData_mdb_attribute *attrs)
{
	 ...
	 * data from mdb_class and replace what we've done here.  Note in
 ...
 *	can mean that its mdb_class or mdb_index row changed.  There are only
 ...
 *	and/or in active use.  We support full replacement of the mdb_class row,
 *	as well as updates of a few simple fields of the mdb_index row.
 *
 ...
 *
 *	If the target index is an index on mdb_class or mdb_index, we'd better have
...
RelationReloadIndexInfo(Relation relation)
{
	 ...
	 * mdb_class yet.  However, a shared index can never have any significant
	...
	/*
	 * Read the mdb_class row
	 *
	 * Don't try to use an indexscan of mdb_class_oid_index to reload the info
	 * for mdb_class_oid_index ...
	 */
	indexOK = (RelationGetRelid(relation) != ClassOidIndexId);
	mdb_class_tuple = ScanPgRelation(RelationGetRelid(relation), indexOK, false);
	if (!HeapTupleIsValid(mdb_class_tuple))
		elog(ERROR, "could not find mdb_class tuple for index %u",
			 RelationGetRelid(relation));
	relp = (Form_mdb_class) GETSTRUCT(mdb_class_tuple);
	...
	RelationParseRelOptions(relation, mdb_class_tuple);
	/* done with mdb_class tuple */
	heap_freetuple(mdb_class_tuple);
	/* We must recalculate physical address in case it changed */
	RelationInitPhysicalAddr(relation);

	/*
	 * For a non-system index, there are fields of the mdb_index row that are
	 * allowed to change, so re-read that row and update the relcache entry.
	 * Most of the info derived from mdb_index (such as support function lookup
...
RelationClearRelation(Relation relation, bool rebuild)
{
	 ...
	 * mdb_class row to see if its relfilenode changed. We do that immediately
	 ...
	 * re-read the mdb_class row to handle possible physical relocation of the
	 * index, and we check for mdb_index updates too.
	 */
	if (relation->rd_rel->relkind == RELKIND_INDEX &&
		relation->rd_refcnt > 0 &&
	...
	else
	{
		 ...
		 * attempt to preserve the mdb_class entry (rd_rel), tupledesc, and
...
#define SWAPFIELD(fldtype, fldname) \
		...
		SWAPFIELD(Form_mdb_class, rd_rel);
...
RelationCacheInvalidate(void)
{
	...
	while ((idhentry = (RelIdCacheEnt *) hash_seq_search(&status)) != NULL)
	{
		...
		else
		{
			 ...
			 * mdb_class goes to the front of rebuildFirstList while
			 * mdb_class_oid_index goes to the back of rebuildFirstList, so
						   ...
						   char relkind)
{
	 ...
	 */
	rel->rd_rel = (Form_mdb_class) palloc0(CLASS_TUPLE_SIZE);
						  ...
						  TransactionId freezeXid, MultiXactId minmulti)
{
	...
	/*
	 * Get a writable copy of the mdb_class tuple for the given relation.
	 */
	...
	/*
	 * Now update the mdb_class row.  However, if we're dealing with a mapped
	 * index, mdb_class.relfilenode doesn't change; instead we have to send the
	...
	/*
	 * Make the mdb_class row change visible, as well as the relation map
 ...
 *		We must at least set up nailed reldescs for mdb_database, mdb_authid,
 *		mdb_auth_members, and mdb_shseclabel. Ideally we'd like to have reldescs
...
RelationCacheInitializePhase2(void)
{
	...
	{
		formrdesc("mdb_database", DatabaseRelation_Rowtype_Id, true,
				  true, Natts_mdb_database, Desc_mdb_database);
		formrdesc("mdb_authid", AuthIdRelation_Rowtype_Id, true,
				  true, Natts_mdb_authid, Desc_mdb_authid);
		formrdesc("mdb_auth_members", AuthMemRelation_Rowtype_Id, true,
				  false, Natts_mdb_auth_members, Desc_mdb_auth_members);
		formrdesc("mdb_shseclabel", SharedSecLabelRelation_Rowtype_Id, true,
				  false, Natts_mdb_shseclabel, Desc_mdb_shseclabel);
...
RelationCacheInitializePhase3(void)
{
		...
		!load_relcache_init_file(false))
	{
		...
		formrdesc("mdb_class", RelationRelation_Rowtype_Id, false,
				  true, Natts_mdb_class, Desc_mdb_class);
		formrdesc("mdb_attribute", AttributeRelation_Rowtype_Id, false,
				  false, Natts_mdb_attribute, Desc_mdb_attribute);
		formrdesc("mdb_proc", ProcedureRelation_Rowtype_Id, false,
				  true, Natts_mdb_proc, Desc_mdb_proc);
		formrdesc("mdb_type", TypeRelation_Rowtype_Id, false,
				  true, Natts_mdb_type, Desc_mdb_type);
...
#define NUM_CRITICAL_LOCAL_INDEXES	7	/* fix if you change list above */
	 ...
	 * need to nail up some indexes on mdb_authid and mdb_auth_members for use
...
#define NUM_CRITICAL_SHARED_INDEXES 6	/* fix if you change list above */
	 ...
	 * faked up relcache entries using formrdesc, then read the real mdb_class
	...
	while ((idhentry = (RelIdCacheEnt *) hash_seq_search(&status)) != NULL)
	{
		...
		/*
		 * If it's a faked-up entry, read the real mdb_class tuple.
		 */
		if (relation->rd_rel->relowner == InvalidOid)
		{
			...
			if (!HeapTupleIsValid(htup))
				elog(FATAL, "cache lookup failed for relation %u",
					 RelationGetRelid(relation));
			relp = (Form_mdb_class) GETSTRUCT(htup);
			...
			if (relation->rd_rel->relowner == InvalidOid)
				elog(ERROR, "invalid relowner in mdb_class entry for \"%s\"",
					 RelationGetRelationName(relation));
		 ...
		 * here.  We don't make any attempt to fix the mdb_class entry, though.
		 */
		 ...
		 * if there is no policy defined in mdb_policy.
		 */
...
/*
 * GetPgClassDescriptor -- get a predefined tuple descriptor for mdb_class
 * GetPgIndexDescriptor -- get a predefined tuple descriptor for mdb_index
 *
 * We need this kluge because we have to be able to access non-fixed-width
 * fields of mdb_class and mdb_index before we have the standard catalog caches
 ...
 */
static TupleDesc
BuildHardcodedDescriptor(int natts, const FormData_mdb_attribute *attrs,
						 bool hasoids)
{
	TupleDesc	result;
	MemoryContext oldcxt;
...
GetPgClassDescriptor(void)
{
	...
	/* Already done? */
	if (pgclassdesc == NULL)
		pgclassdesc = BuildHardcodedDescriptor(Natts_mdb_class,
											   Desc_mdb_class,
											   true);
...
GetPgIndexDescriptor(void)
{
	...
	/* Already done? */
	if (pgindexdesc == NULL)
		pgindexdesc = BuildHardcodedDescriptor(Natts_mdb_index,
											   Desc_mdb_index,
											   false);
...
AttrDefaultFetch(Relation relation)
{
	...
	{
		Form_mdb_attrdef adform = (Form_mdb_attrdef) GETSTRUCT(htup);
...
CheckConstraintFetch(Relation relation)
{
	...
	{
		Form_mdb_constraint conform = (Form_mdb_constraint) GETSTRUCT(htup);
		Datum		val;
		bool		isnull;
		char	   *s;
 ...
 *
 * The index list is created only if someone requests it.  We scan mdb_index
 ...
 * it is the mdb_class OID of a unique index on OID when the relation has one,
 * and InvalidOid if there is no such index.
 *
 * In exactly the same way, we update rd_replidindex, which is the mdb_class
...
RelationGetIndexList(Relation relation)
{
	...
	{
		Form_mdb_index index = (Form_mdb_index) GETSTRUCT(htup);
		Datum		indclassDatum;
		oidvector  *indclass;
		bool		isnull;
		...
		if (!IndexIsValid(index) || !index->indisunique ||
			!index->indimmediate ||
			!heap_attisnull(htup, Anum_mdb_index_indpred))
			continue;
 ...
 *
 * Use an index scan on mdb_constraint to load in FK definitions,
...
RelationGetFKeyList(Relation relation)
{
	...
	/* Prepare to scan mdb_constraint for entries having conrelid = this rel. */
	ScanKeyInit(&skey,
				Anum_mdb_constraint_conrelid,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(RelationGetRelid(relation)));
	...
	while (HeapTupleIsValid(htup = systable_getnext(conscan)))
	{
		Form_mdb_constraint constraint = (Form_mdb_constraint) GETSTRUCT(htup);
 ...
 * messages.  In practice it is only used on mdb_class (see REINDEX).
 *
...
/*
 * RelationGetOidIndex -- get the mdb_class OID of the relation's OID index
 *
 ...
 *
 * We cache the result of transforming mdb_index.indexprs into a node tree.
...
RelationGetIndexExpressions(Relation relation)
{
	...
	if (relation->rd_indextuple == NULL ||
		heap_attisnull(relation->rd_indextuple, Anum_mdb_index_indexprs))
		return NIL;
 ...
 *
 * We cache the result of transforming mdb_index.indpred into an implicit-AND
...
RelationGetIndexPredicate(Relation relation)
{
	...
	/* Quick exit if there is nothing to do. */
	if (relation->rd_indextuple == NULL ||
		heap_attisnull(relation->rd_indextuple, Anum_mdb_index_indpred))
		return NIL;
...
RelationGetIndexAttrBitmap(Relation relation, IndexAttrBitmapKind attrKind)
{
	...
	foreach(l, indexoidlist)
	{
		...
		/* Extract index key information from the index's mdb_index row */
						 ...
						 uint16 **strategies)
{
	...
	*strategies = strats = (uint16 *) palloc(sizeof(uint16) * ncols);
	...
	/*
	 * Search mdb_constraint for the constraint associated with the index. To
 ...
 *		use an index on mdb_attribute, for example, but in order to do so,
 *		we must have read mdb_attribute for the attributes in the index,
...
load_relcache_init_file(bool shared)
{
	...
	for (relno = 0;; relno++)
	{
		...
		relform = (Form_mdb_class) palloc(len);
		...
		if (rel->rd_rel->relkind == RELKIND_INDEX)
		{
			...
			rel->rd_index = (Form_mdb_index) GETSTRUCT(rel->rd_indextuple);
		 ...
		 * case the mdb_internal.init file was copied from some other database
		 * by CREATE DATABASE.
		 */
...
write_relcache_init_file(bool shared)
{
	...
	while ((idhentry = (RelIdCacheEnt *) hash_seq_search(&status)) != NULL)
	{
		...
		Form_mdb_class relform = rel->rd_rel;
...
{
	const char *tblspcdir = "mdb_tblspc";
	DIR		   *dir;
	struct dirent *de;
	char		path[MAXPGPATH];
