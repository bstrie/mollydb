 ...
 * mdb_signal_queue must be volatile since it is changed by the signal
 * handling thread and inspected without any lock by the main thread.
 * mdb_signal_mask is only changed by main thread so shouldn't need it.
 */
volatile int mdb_signal_queue;
int			mdb_signal_mask;
...
/*
 * mdb_signal_crit_sec is used to protect only mdb_signal_queue. That is the only
...
static DWORD WINAPI mdb_signal_thread(LPVOID param);
static BOOL WINAPI mdb_console_handler(DWORD dwCtrlType);


/*
 * mdb_usleep --- delay the specified number of microseconds, but
...
pgwin32_signal_initialize(void)
{
	...
	signal_thread_handle = CreateThread(NULL, 0, mdb_signal_thread, NULL, 0, NULL);
	if (signal_thread_handle == NULL)
		ereport(FATAL,
				(errmsg_internal("could not create signal handler thread")));

	/* Create console control handle to pick up Ctrl-C etc */
	if (!SetConsoleCtrlHandler(mdb_console_handler, TRUE))
		ereport(FATAL,
...
pgwin32_dispatch_queued_signals(void)
{
	...
	while ((exec_mask = UNBLOCKED_SIGNAL_QUEUE()) != 0)
	{
		...
		for (i = 1; i < MDB_SIGNAL_COUNT; i++)
		{
			...
			{
				/* Execute this signal */
				pqsigfunc	sig = mdb_signal_array[i];

				if (sig == SIG_DFL)
					sig = mdb_signal_defaults[i];
				mdb_signal_queue &= ~sigmask(i);
				if (sig != SIG_ERR && sig != SIG_IGN && sig != SIG_DFL)
				{
					LeaveCriticalSection(&mdb_signal_crit_sec);
					sig(i);
					EnterCriticalSection(&mdb_signal_crit_sec);
...
pqsigsetmask(int mask)
{
	...
	prevmask = mdb_signal_mask;
	mdb_signal_mask = mask;
...
mdb_queue_signal(int signum)
{
	...
	EnterCriticalSection(&mdb_signal_crit_sec);
	mdb_signal_queue |= sigmask(signum);
	LeaveCriticalSection(&mdb_signal_crit_sec);
...
static DWORD WINAPI
mdb_signal_dispatch_thread(LPVOID param)
{
	...
	mdb_queue_signal(sigNum);
	return 0;
}

/* Signal handling thread */
static DWORD WINAPI
mdb_signal_thread(LPVOID param)
{
	char		pipename[128];
	...
	for (;;)
	{
		...
		if (fConnected)
		{
			...
			hThread = CreateThread(NULL, 0,
						  (LPTHREAD_START_ROUTINE) mdb_signal_dispatch_thread,
								   (LPVOID) pipe, 0, NULL);
...
static BOOL WINAPI
mdb_console_handler(DWORD dwCtrlType)
{
