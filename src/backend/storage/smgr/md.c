...
mdcreate(SMgrRelation reln, ForkNumber forkNum, bool isRedo)
{
	...
	fd = PathNameOpenFile(path, O_RDWR | O_CREAT | O_EXCL | MDB_BINARY, 0600);
	...
	if (fd < 0)
	{
		...
		if (isRedo || IsBootstrapProcessingMode())
			fd = PathNameOpenFile(path, O_RDWR | MDB_BINARY, 0600);
		if (fd < 0)
		{
			  ...
			  int oflags)
{
	...
	/* open the file */
	fd = PathNameOpenFile(fullpath, O_RDWR | MDB_BINARY | oflags, 0600);
