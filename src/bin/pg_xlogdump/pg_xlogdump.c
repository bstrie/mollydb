/*-------------------------------------------------------------------------
 *
 * mdb_xlogdump.c - decode and display WAL
 *
 * Copyright (c) 2013-2016, MollyDB Global Development Group
 *
 * IDENTIFICATION
 *		  src/bin/mdb_xlogdump/mdb_xlogdump.c
 *-------------------------------------------------------------------------
 */
...
static void fatal_error(const char *fmt,...) mdb_attribute_printf(1, 2);
...
split_path(const char *path, char **dir, char **fname)
{
	...
	if (sep != NULL)
	{
		...
		*fname = mdb_strdup(sep + 1);
	}
	/* local directory */
	else
	{
		*dir = NULL;
		*fname = mdb_strdup(path);
	}
}
								 ...
								 long_options, &optindex)) != -1)
	{
		switch (option)
		{
			...
			case 'n':
				...
				break;
			case 'p':
				private.inpath = mdb_strdup(optarg);
				break;
			case 'r':
				{
					int			i;

					if (mdb_strcasecmp(optarg, "list") == 0)
					{
						print_rmgr_list();
						exit(EXIT_SUCCESS);
					}
					...
					for (i = 0; i <= RM_MAX_ID; i++)
					{
						if (mdb_strcasecmp(optarg, RmgrDescTable[i].rm_name) == 0)
						{
