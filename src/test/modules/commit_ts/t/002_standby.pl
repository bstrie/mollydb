...
my $standby_ts = $standby->safe_psql('mollydb',
	qq{select ts.* from mdb_class, mdb_xact_commit_timestamp(xmin) ts where relname = 't10'});
...
my ($ret, $standby_ts_stdout, $standby_ts_stderr) = $standby->psql('mollydb',
	'select ts.* from mdb_class, mdb_xact_commit_timestamp(xmin) ts where relname = \'t10\'');
