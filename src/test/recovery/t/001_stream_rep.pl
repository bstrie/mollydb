...
my $result =
  $node_standby_1->safe_psql('mollydb', "SELECT count(*) FROM tab_int");
print "standby 1: $result\n";
is($result, qq(1002), 'check streamed content on standby 1');

$result = $node_standby_2->safe_psql('mollydb', "SELECT count(*) FROM tab_int");
print "standby 2: $result\n";
is($result, qq(1002), 'check streamed content on standby 2');

# Check that only READ-only queries can run on standbys
is($node_standby_1->psql('mollydb', 'INSERT INTO tab_int VALUES (1)'), 3, 'Read-only queries on standby 1');
is($node_standby_2->psql('mollydb', 'INSERT INTO tab_int VALUES (1)'), 3, 'Read-only queries on standby 2');
