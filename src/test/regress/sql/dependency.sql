...
-- the error message detail here would include a mdb_toast_nnn name that
...
CREATE FUNCTION deptest_func() RETURNS void LANGUAGE plmdb
  AS $$ BEGIN END; $$;
...
-- When reassigning ownership of a composite type, its mdb_class entry
-- should match
CREATE TYPE deptest_t AS (a int);
SELECT typowner = relowner
FROM mdb_type JOIN mdb_class c ON typrelid = c.oid WHERE typname = 'deptest_t';
