...
-- mdb_type, mdb_class, mdb_attribute, mdb_range.
--
...
-- Look for illegal values in mdb_type fields.
...
-- NOTE: as of 9.1, this check finds mdb_node_tree, smgr, and unknown.

SELECT p1.oid, p1.typname
FROM mdb_type as p1
WHERE p1.typtype not in ('c','d','p') AND p1.typname NOT LIKE E'\\_%'
    AND NOT EXISTS
    (SELECT 1 FROM mdb_type as p2
...
FROM   mdb_type p1 LEFT JOIN mdb_type p2 ON (p1.typarray = p2.oid)
WHERE  p1.typarray <> 0 AND
       (p2.oid IS NULL OR p2.typelem <> p1.oid OR p2.typlen <> -1);

-- Look for range types that do not have a mdb_range entry
SELECT p1.oid, p1.typname
FROM mdb_type as p1
WHERE p1.typtype = 'r' AND
   NOT EXISTS(SELECT 1 FROM mdb_range r WHERE rngtypid = p1.oid);

-- Look for range types whose typalign isn't sufficient
SELECT p1.oid, p1.typname, p1.typalign, p2.typname, p2.typalign
FROM mdb_type as p1
     LEFT JOIN mdb_range as r ON rngtypid = p1.oid
     LEFT JOIN mdb_type as p2 ON rngsubtype = p2.oid
WHERE p1.typtype = 'r' AND
...
FROM mdb_type AS p1, mdb_proc AS p2
...
FROM mdb_type AS p1 LEFT JOIN mdb_type AS p2 ON p1.typbasetype = p2.oid
...
FROM mdb_type AS p1, mdb_proc AS p2, mdb_proc AS p3
...
SELECT p1.oid, p1.typname, p2.oid, p2.typname
FROM mdb_type AS p1, mdb_type AS p2
...
FROM mdb_type d JOIN mdb_type t ON d.typbasetype = t.oid
...
FROM mdb_type t LEFT JOIN mdb_range r on t.oid = r.rngtypid
...
-- Look for illegal values in mdb_class fields

SELECT p1.oid, p1.relname
FROM mdb_class as p1
...
-- Look for illegal values in mdb_attribute fields

SELECT p1.attrelid, p1.attname
FROM mdb_attribute as p1
...
FROM mdb_attribute AS p1, mdb_class AS p2
WHERE p1.attrelid = p2.oid AND p1.attnum > p2.relnatts;

-- Detect missing mdb_attribute entries: should have as many non-system
-- attributes as parent relation expects

SELECT p1.oid, p1.relname
FROM mdb_class AS p1
WHERE p1.relnatts != (SELECT count(*) FROM mdb_attribute AS p2
                      WHERE p2.attrelid = p1.oid AND p2.attnum > 0);

-- Cross-check against mdb_type entry
-- NOTE: we allow attstorage to be 'plain' even when typstorage is not;
-- this is mainly for toast tables.

SELECT p1.attrelid, p1.attname, p2.oid, p2.typname
FROM mdb_attribute AS p1, mdb_type AS p2
...
-- Look for illegal values in mdb_range fields.

SELECT p1.rngtypid, p1.rngsubtype
FROM mdb_range as p1
...
FROM mdb_range p1 JOIN mdb_type t ON t.oid = p1.rngsubtype
...
FROM mdb_range p1 JOIN mdb_opclass o ON o.oid = p1.rngsubopc
WHERE o.opcmethod != 403 OR
    ((o.opcintype != p1.rngsubtype) AND NOT
     (o.opcintype = 'mdb_catalog.anyarray'::regtype AND
